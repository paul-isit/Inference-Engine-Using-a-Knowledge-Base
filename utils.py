...
#     s2 = s.copy()
#     s2[var] = val
#     print(f"this is from the extend method: {s2}")
#     return s2
...
infix_ops = '<=> <= =>'.split()
infix_ops.sort(key=len, reverse=True)
...
def expr_handle_infix_ops(x):
    ...
    # Create a regular expression pattern that matches any of the infix operators
    pattern = re.compile('|'.join(map(re.escape, infix_ops)))
    # Use the pattern to replace all matches with the desired format
    x = pattern.sub(replacer, x)
    return x
